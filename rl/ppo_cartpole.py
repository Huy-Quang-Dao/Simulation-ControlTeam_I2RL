...
def make_env(env_id, idx, capture_video, run_name, seed=None):
    def thunk():
...
if __name__ == "__main__":
    ...
    eval_env = gym.vector.SyncVectorEnv(
        [make_env(args.env_id, 0, capture_video=True, run_name=f"{run_name}-eval", seed=args.seed)]
    )
    ...
    while episode_count < num_eval_episodes:
        with torch.no_grad():
            # Use deterministic action (argmax of logits) for evaluation
